...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
    const handler = createQueryDataHandler("IDT_DATA_DO", "idt");

    server.registerTool(
        "idt_query_data",
        {
            title: "Query Staged IDT Data",
            description:
                "Query staged IDT data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
